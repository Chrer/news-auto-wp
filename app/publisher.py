...
from .feeds import read_feed, infer_category_from_url
...
def prepare_item_content(item: dict, cfg: dict) -> tuple[str, str, str | None, datetime | None]:
    ...
    if should_copy_full:
        ...
        if len(paragraphs) < MIN_PARAGRAPHS_FULL_ARTICLE:
            # Permite una sola pieza extensa si el sitio entrega todo como bloque continuo.
            if not (len(paragraphs) == 1 and len(paragraphs[0]) > 450):
...
def run_once(source_id: str | None = None, target_category: str | None = None, progress_callback=None) -> dict:
    ...
    for source, item in interleave_sources(source_items):
        ...
        if item["url"] in seen_run_urls:
            skipped.append({"title": clean_text_global(item.get("title")), "source": source_name, "reason": "duplicada en esta ejecución"})
            if source_name in source_report:
                source_report[source_name]["skipped"] += 1
            continue
        seen_run_urls.add(item["url"])
        detected_url_category = item.get("url_category") or infer_category_from_url(item["url"])
        if detected_url_category:
            item["target_category"] = detected_url_category
            item["default_category"] = detected_url_category
